...
class LitSQUIM(LitWAWEnetModule):
    def __init__(
        self,
        learning_rate: float,
        *args: Any,
...
class LitSQUIMSubjIU(LitWAWEnetModule):
    def __init__(
        self,
        learning_rate: float,
        *args: Any,
        ...
        **kwargs: Any,
        ...
        # normalize SQUIM predictions so all our analysis code works properly
        self.normalizer_transforms = transforms.Compose(
            [get_normalizer_class("ScaledMOS", 0)]
        )
    ...
    def test_step(
        ...
        **kwargs: Any,
        ...
        impairment = batch["impairment"]
        # predict
        # this model doesn't accept batches for some reason, so we're doin' it
        # live
        predictions = list()
        for ind in range(x.shape[0]):
            mos = self.model(x[ind, :, :], nmr[ind, :, :])
            predictions.append(torch.Tensor([mos]))
...
class LitSQUIMSubjNISQA(LitWAWEnetModule):
    def __init__(
        ...
        **kwargs: Any,
        ...
        # normalize SQUIM predictions so all our analysis code works properly
        # TODO: lots of repeated code to change just "mos" below. figure out how to
        #       pass it through as an arg.
        self.normalizer_transforms = transforms.Compose(
            [get_normalizer_class("mos", 0)]
        )
